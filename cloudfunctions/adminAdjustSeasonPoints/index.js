// ABOUTME: Adds a point adjustment entry for a player in a season.
// ABOUTME: Adjustments are aggregated with event points in getSeasonStats.
...
exports.main = async (event, context) => {
  ...
  if (!seasonId || !playerId || deltaPoints === undefined || deltaPoints === null) {
    throw new Error('MISSING_FIELDS');
  }

  const now = new Date().toISOString();
  await db.collection('season_point_adjustments').add({
    data: {
      seasonId,
      playerId,
